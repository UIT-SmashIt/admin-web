...
import {
  ...
  INITIAL_BOOKINGS, PAYMENT_STATUS_CONFIG, COURTS, fmt,
} from './LichDatTypes';
...
function DetailModal({ booking, onClose, onStatusChange }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 18, width: 480, maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(0,0,0,0.18)', fontFamily: "'Be Vietnam Pro', sans-serif" }} onClick={e => e.stopPropagation()}>
        {/* Header */}
        ...
        <div style={{ padding: '20px 24px' }}>
          ...
          {isCommunity && (
            <div style={{ marginBottom: 14, padding: '12px 14px', borderRadius: 10, background: '#F3E8FF', border: '1px solid #D8B4FE' }}>
          ...
          {isCommunity && booking.caption && (
            <div style={{ marginBottom: 14, padding: '12px 14px', borderRadius: 10, background: '#FAFAFA', border: '1px solid #e8e8e8' }}>
